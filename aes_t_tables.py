...
# t_tables = correct_t_tables[3][255]
state = enc_round_state_in[0]
roundkey = enc_round_key_in[0]
# print(state)
# print(hex(228))
...
def subBytes(state):
    for i in range(len(state)):
        state[i] = hex(state[i])[2:].zfill(2)
        state[i] = sbox[(int(state[i][0], 16) * 16 + int(state[i][1], 16))]
    return state
...
def enc_round(t_tables, state, roundkey):
    ...
    for i in range(len(state)):
        state[i] = hex(t_tables[i % 4][state[i]])[2:]

    for i in range(len(state)):
        state[i] = int(state[i][:2], 16) ^ int(state[i][2:4], 16) ^ int(state[i][4:6], 16) ^ int(state[i][6:], 16) ^ 0x1b
        # print(hex(int(state[4][:2],16) ^ int(state[4][2:4],16) ^ int(state[4][4:6],16) ^ int(state[4][6:],16) ^ 0x1b)[2:].zfill(2))
...
enc_round(t_tables, state, roundkey)
#print(state[0])
